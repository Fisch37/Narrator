...
class ConfirmationView(View):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.__confirm.style = confirm_style
        self.__cancel.style = cancel_style
    
    @button(label="Confirm", row=4)
    async def __confirm(self, interaction: Interaction, _):
        await interaction.response.defer()
        self.__future.set_result(True)
        self.stop()
    
    @button(label="Cancel", row=4)
    async def __cancel(self, interaction: Interaction, _):
        await interaction.response.defer()
